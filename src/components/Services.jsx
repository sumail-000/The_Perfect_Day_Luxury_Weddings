import { Flower2, Stars, MapPin } from "lucide-react";
...
function ServiceCard({ icon: Icon, title, desc, accent }) {
  return (
    <div className="group rounded-[2rem] border bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
      ...
      <h3 className="font-display text-2xl font-semibold text-[#453339]">{title}</h3>
      <p className="mt-3 font-body leading-7 text-[#6b5c63]">{desc}</p>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl px-6 py-28 lg:px-10">
        <div className="mb-16 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="max-w-3xl">
            <div className={`font-script text-5xl ${scriptClass}`}>{scriptText}</div>
          ...
          {image && (
            <div className="hidden overflow-hidden rounded-[2rem] lg:block">
              <img src={image} alt={scriptText} className="h-48 w-64 object-cover" />
            </div>
          )}
        </div>
